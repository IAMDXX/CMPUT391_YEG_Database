...
#with open('wp.csv', 'wb') as csvfile3, open('wt.csv', 'wb') as csvfile4, open('wf.csv', 'wb') as csvfile5:
        ...
        if element.tag == 'way':
            state = 1
            if wid != str(element.attrib['id']) and wid != 0:
                ...
                if way_pts[0] == way_pts[-1]:
                    close = True     
                writer5.writerow({'id': wid, 'closed': str(close)})
                
                for i in range(len(way_pts)):       
                   # row = "{'id': " + str(element.attrib['id'])+", 'ordi': "+ str(i) + ", nodeid': "+ str(way_p[i])+"}"
                    writer3.writerow( {'id': wid, 'ordi': str(i), 'nodeid': str(way_pts[i])} )
                
                for i in range(len(k)):
                    writer4.writerow({'id': wid, 'k': k[i].encode('utf-8'), 'v': v[i].encode('utf-8')} )             
        ...
        if element.tag == 'nd' and discard == False:
            if (element.attrib['ref'] in nodes.keys()):
                way_pts.append(element.attrib['ref'])
            else:
                discard = True
                continue
            element.clear()
            
        if element.tag == 'tag' and state == 1 and discard == False:
            k[ordi] = (element.attrib['k'])
            v[ordi] = (element.attrib['v'])
            ordi += 1
            element.clear()
